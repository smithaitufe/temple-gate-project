defmodule PortalApi.V1.StudentCourseController do
  ...
  def index(conn, params) do
    ...
    |> build_student_course_query(Map.to_list(params))
    |> Repo.all
    |> preload_models
  ...
  defp build_student_course_query(query, []), do: query
  defp build_student_course_query(query, [{"level_id", level_id} | tail]) do
    query
    |> Ecto.Query.join(:left, [sc], c in assoc(sc, :course))
    |> Ecto.Query.where([_, c], c.level_id == ^level_id)
    |> build_student_course_query(tail)
  end
  defp build_student_course_query(query, [{"student_id", student_id} | tail]) do
    query
